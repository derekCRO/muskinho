...
function digitsCountersAnimate(digitsCounter) {
  ...
  const duration = parseInt(digitsCounter.dataset.duration)
    ? parseInt(digitsCounter.dataset.duration * 1000)
    : 1000;
  const dataCount = digitsCounter.dataset.count;
  const endValue = dataCount
    ? dataCount.replace(/\s/g, "")
    : digitsCounter.innerHTML.replace(/\s/g, "");
  ...
  const step = (timestamp) => {
    ...
    digitsCounter.innerHTML = numberWithSeparator(
